...
let apiBooks = {}
// let apiBooks = storageService.query(BOOKS_API_KEY).then(res => {
//   if (!res || !res.length) {
//     return []
//   }
//   else {
...
function getBooksFromApi(searchKey) {
  ...
  if (gCatch[searchKey]) {
    ...
    return Promise.resolve(gCatch[searchKey])
  }
  console.log('from api');
  return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchKey}`)
    .then((res) => {
      apiBooks = res.data.items
      gCatch[searchKey] = apiBooks
      storageService.post(BOOKS_API_KEY, gCatch)
      return Promise.resolve(gCatch[searchKey])
    })
}

function searceByKey(keyWord) {
